...
import com.redmagic.undefinedapi.API
 ...
 */
fun sync(runnable: BukkitRunnable.() -> Unit): BukkitTask = createRunnable(runnable).runTask(API.plugin)
/**
 ...
 */
fun async(runnable: BukkitRunnable.() -> Unit): BukkitTask = createRunnable(runnable).runTaskAsynchronously(API.plugin)
...
fun repeatingTask(delay: Int, period: Int, times: Int = -1, unit: TimeUnit = TimeUnit.TICKS, async: Boolean = false, runnable: BukkitRunnable.() -> Unit): BukkitTask{
    return if (async){
        createRunnable(times, runnable).runTaskTimerAsynchronously(API.plugin, unit.toTicks(delay.toLong()), unit.toTicks(period.toLong()))
    }else{
        createRunnable(times ,runnable).runTaskTimer(API.plugin, unit.toTicks(delay.toLong()), unit.toTicks(period.toLong()))
    }
}
/**
